...
# Dedicated folding bioir kernel image build backend.
#
# Operator usage goes through:
#   ./containers/scripts/build.sh folding bioir [docker-build-args...]
#
# Direct backend usage:
#   ./containers/folding/bioir/build.sh [image] [docker-build-args...]
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO="$(git -C "$HERE" rev-parse --show-toplevel)"
IMAGE="${1:-bspp-orchestration:folding-bioir}"
if [[ $# -gt 0 ]]; then
  shift
fi
...
[[ -z "$(git -C "$REPO" status --porcelain=v1 --untracked-files=all)" ]] || {
  echo "folding bioir image builds require a clean committed source checkout" >&2
  exit 2
}
...
BIOIR_VERSION="$(jq -r '.bioir_version' "$HERE/image-lock.json")"
...
# Python 3.12 is provisioned inside the Dockerfile via apt-get (the base image
# nvidia/cuda:13.0.3-devel-ubuntu24.04 ships neither python3 nor pip). The
...
jq -n \
  ...
  --arg bioir_version "$BIOIR_VERSION" \
  ...
  '{schema_version: 1, source_commit: $source_commit, image_lock_sha256: $image_lock_sha256,
    ...
    bioir_version: $bioir_version, cuda_version: $cuda_version}' \
  > "$CONTEXT/folding-bioir-image.json"
...
"${ENGINE[@]}" build \
  ...
  --build-arg "BIOIR_VERSION=$BIOIR_VERSION" \
