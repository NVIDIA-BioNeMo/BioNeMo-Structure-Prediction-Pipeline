...
# Dedicated folding runtime image build backend.
#
# Operator usage goes through:
#   ./containers/scripts/build.sh folding runtime [docker-build-args...]
#
# Direct backend usage:
#   ./containers/folding/runtime/build.sh [image] [docker-build-args...]
#
# The pinned pixi binary is downloaded and sha256-verified inside the
# Dockerfile (via --build-arg from image-lock.json); `pixi install` resolves
# non-locked. No host-side pixi and no committed pixi.lock are required.
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO="$(git -C "$HERE" rev-parse --show-toplevel)"
IMAGE="${1:-bspp-orchestration:folding-runtime}"
if [[ $# -gt 0 ]]; then
  shift
fi
...
[[ -z "$(git -C "$REPO" status --porcelain=v1 --untracked-files=all)" ]] || {
  echo "folding runtime image builds require a clean committed source checkout" >&2
  exit 2
}
...
# Verify the base image digest is not the placeholder (runtime reuses a real digest).
...
jq -n \
  ...
  > "$CONTEXT/folding-runtime-image.json"
