...
"""Renderer contract 2 for postprocessing V2 authorities."""
...
def render_postprocessing_action_script_v2_contract2(
    authority: PostprocessingRenderInput,
    action: PostprocessingRuntimeAction,
) -> str:
    """Render a V2 action with the value-free AWS shared-profile boundary."""
...
def _render_postprocessing_action_script_body(
    ...
    command = render_governed_srun(image, mounts=mount_text, bootstrap_args=tuple(bootstrap_args))
...
def _postprocessing_governed_mounts(
    ...
    *,
    ...
    if _aws_profile_name(authority) is not None:
        aws_profile_source = Path.home() / ".aws"
        mounts.extend(
            (
                (
                    str(aws_profile_source / "credentials"),
                    f"{_AWS_SHARED_CREDENTIALS_CONTAINER_ROOT / 'credentials'}:ro",
                ),
                (
                    str(aws_profile_source / "config"),
